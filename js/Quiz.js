...
async function startQuiz() {
  ...
  // Show loading toast since we are generating AI questions
  showToast('🤖 Generating questions with Groq AI...', 'info', 3000);

  // Load questions from backend
  let bank = [];
  try {
    const data = await apiCall(`/api/questions/${state.subject}`);
    if (data && Array.isArray(data) && data.length > 0) {
      bank = data;
    } else {
      bank = QUESTION_BANK[state.subject] || [];
    }
  } catch(e) {
    bank = QUESTION_BANK[state.subject] || [];
  }
  ...
  if (bank.length === 0) {
    showToast(`No questions available for ${state.subject}.`, 'warning', 4000);
    bank = [
